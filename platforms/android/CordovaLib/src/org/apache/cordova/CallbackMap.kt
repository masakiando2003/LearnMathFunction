...
class CallbackMap {
    ...
    private val callbacks: SparseArray<Pair<CordovaPlugin, Integer>>

    init {
        this.callbacks = SparseArray<Pair<CordovaPlugin, Integer>>()
    }
     ...
     */
    @Synchronized
    fun registerCallback(receiver: CordovaPlugin, requestCode: Int): Int {
        val mappedId = this.currentCallbackId++
        callbacks.put(mappedId, Pair<CordovaPlugin, Integer>(receiver, requestCode))
        return mappedId
    }
     ...
     */
    @Synchronized
    fun getAndRemoveCallback(mappedId: Int): Pair<CordovaPlugin, Integer> {
        val callback = callbacks.get(mappedId)
