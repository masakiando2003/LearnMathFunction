...
class NativeToJsMessageQueueTest {

    internal var queue = NativeToJsMessageQueue()
    private val TEST_CALLBACK_ID = "MessageQueueTest"
    ...
    @Test
    fun testEnabledBridge() {
        val bridge: NativeToJsMessageQueue.BridgeMode
        bridge = NativeToJsMessageQueue.NoOpBridgeMode()
    ...
    fun testPopAndEncode() {
        ...
        val resultString = queue.popAndEncode(false)
    ...
    fun testBasicPopAndEncodeAsJs() {
        ...
        queue.addPluginResult(result, TEST_CALLBACK_ID)
        assertFalse(queue.isEmpty())
        val resultString = queue.popAndEncodeAsJs()
    ...
    //platform specific NativeToJs code
    @Test
    fun testStringPopAndEncodeAsJs() {
        val bridge: NativeToJsMessageQueue.BridgeMode
        bridge = NativeToJsMessageQueue.NoOpBridgeMode()
        queue.addBridgeMode(bridge)
        queue.setBridgeMode(0)

        val result = PluginResult(PluginResult.Status.OK, "String Plugin Result")
        queue.addPluginResult(result, TEST_CALLBACK_ID)
        assertFalse(queue.isEmpty())
        val resultString = queue.popAndEncodeAsJs()
        assertTrue(resultString.startsWith("cordova.callbackFromNative"))
    ...
    fun testJsonPopAndEncodeAsJs() {
        ...
        val result = PluginResult(PluginResult.Status.OK, `object`)
        queue.addPluginResult(result, TEST_CALLBACK_ID)
        assertFalse(queue.isEmpty())
        val resultString = queue.popAndEncodeAsJs()
        assertTrue(resultString.startsWith("cordova.callbackFromNative"))
    ...
    //platform specific NativeToJs code
    @Test
    fun testMultipartPopAndEncodeAsJs() {
        val multiparts = ArrayList<PluginResult>()
        for (i in 0..4) {
            multiparts.add(PluginResult(PluginResult.Status.OK, i))
        }
        val multipartresult = PluginResult(PluginResult.Status.OK, multiparts)
        val queue = NativeToJsMessageQueue()
        queue.addBridgeMode(NativeToJsMessageQueue.NoOpBridgeMode())
        queue.setBridgeMode(0)
        queue.addPluginResult(multipartresult, "37")
        val result = queue.popAndEncodeAsJs()
        assertEquals(result, "cordova.callbackFromNative('37',true,1,[0,1,2,3,4],false);")
    }

    @Test
    fun testNullPopAndEncodeAsJs() {
        val queue = NativeToJsMessageQueue()
        queue.addBridgeMode(NativeToJsMessageQueue.NoOpBridgeMode())
        queue.setBridgeMode(0)

        val result = PluginResult(PluginResult.Status.OK, null as String?)
        queue.addPluginResult(result, TEST_CALLBACK_ID)
        assertFalse(queue.isEmpty())
        val resultString = queue.popAndEncodeAsJs()
